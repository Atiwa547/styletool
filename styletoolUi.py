...
class StyleToolDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(
            '''
                QDialog {
                    background-color: #A124F2;
                }
            '''
        )
        self.imageLabel = QtWidgets.QLabel()
        self.imagePixmap = QtGui.QPixmap(f'{IMAGE_DIR}/plab.png')
        ...
        self.nameLineEdit.setStyleSheet(
            '''
                QLineEdit {
                    background-color: qlineargradient(x1:0, y1:0, x2:0 y2:1, stop:0 blue, stop:1 navy);
                    color: white;
                    border-radius: 8px;
                    font-family: Arial;
                    font-weight: bold;
